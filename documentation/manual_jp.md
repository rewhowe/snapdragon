...
一部の組み込み関数以外、関数の引数はコピーとして渡されます（詳しくは「ビルトイン関数」より見てください）。外に定義した変数の値は読み込むことが可能ですが、代入が不可能です。引数または変数名が外のに被る際、関数のスコープが自分のコピーを仕様します。関数呼出終了後まで残したい値があれば、返す（下記のセクション）または特別グローバル`あれ`に代入することが推薦です。詳しくは「それ / あれ」より見てください。
...
2つの引数の際、両方は変数または数値のどれかを使えます(浮動小数点数は整数化されます）。変数に対しては数値的であるべきというのですが、妥当性の確認がありません。浮動小数点は定数にキャストされます。更に、その引数は始点と終点と見なす「から」と「まで」という助詞を順番自由に使います。終点が含まれません。始点と終点を入れ替えることで逆順の反復が可能です。
...
値を返して終了する際、スクリプトの終了コードがデータ型に対して決まります。
...
`エラー`を標準エラー出力に送信し、例外を発生させる。ビックリマークを付けると、引数が不正な際のみエラーが抑えられる。
...
| 引数                                         | 戻り値           | ひらがな可能   |
| -------------------------------------------- | ---------------- | -------------- |
| `要素`: 型自由<br>`対象列`: 配列または文字列 | 抜いた要素の配列 | `全部ぬく`のみ |
...
| 引数                                         | 戻り値   | ひらがな可能   |
| -------------------------------------------- | -------- | -------------- |
| `要素`: 型自由<br>`対象列`: 配列または文字列 | `対象列` | `おしこむ`のみ |
...
`要素`を`対象列`の先頭（指数：0）に押し込む。対象列`が文字列の際：`要素`は文字列に限る。
