...
2つの引数の際、両方は変数または数値のどれかを使えます。変数に対しては数値的であるべきというのですが、正当性の確認がありません。更に、その引数は始点と終点と見なす「から」と「まで」という助詞を順番自由に使います。
...
コンテナーのようなオブジェクトの中身に対して繰り返すのは次のようなフォーマット：`【オブジェクト】に 対して 繰り返す`。そのオブジェクトは配列系の変数または文字列であるべきなのですが、前者の際は正当性の確認がありません。`対して`はひらがなでも書けます。
