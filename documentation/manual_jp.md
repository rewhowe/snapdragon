...
一部の組み込み関数以外、関数の引数はコピーとして渡されます（詳しくは「[ビルトイン関数](#ビルトイン関数組み込み関数)」より参照してください）。外に定義した変数の値は読み込むことが可能ですが、代入が不可能です。引数または変数名が外のに被る際、関数のスコープが別のコピーを使用します。関数呼出終了後まで残したい値があれば、返す（下記のセクション）または特別グローバル`あれ`に代入することが推薦です。詳しくは「[それ / あれ](#それ--あれ)」より参照してください。
...
比2は他方の比較対象と接尾の`と`、`より`、`以上`、`以下`のどれか。

比3は`ならば`、`なら`、`でなければ`、`等しければ`、`大きければ`、`小さければ`のどれか。`ならば`と`なら`は動作的に同様です。

比較演算子となる比2と比3の組み合わせは次のフォーマット：`【バリュー】【比2】 【比3】`。
...
`Ａ`と`Ｂ`のデータ型が違えば、比較文の結果が常に偽となります。
...
更に、一つの値にハテナマーク（全角・半角可能）を付けると、ブーリアン型にキャストされた値に対する条件分岐が可能です。該当すうる比3は`ならば`または`でなければ`です。
...
関数の呼び出しにエラーが発生したらヌルが返されます。呼び出しの命令の最後にビックリマーク（全角・半角可能）を付けるとエラーが浮かび上がります（詳しくは「[例外処理](#例外処理)」より参照してください）。
...
`エラー`を標準エラー出力に送信し、例外を発生させる。引数が不正な際、他の関数と同様にエラーが抑えられる。ビックリマークを付けるとエラーが浮かび上がる。詳しくは「[ビックリマーク（感嘆符）](#ビックリマーク感嘆符)」より参照してください。
