...
予約語に一致したり無効文字を含むの以外、変数名は殆ど無制限です。無効文字：`\n`（二文字、バックすらっし＋半角「n」）、`￥ｎ`（二文字、円記号＋全角「ｎ」）、`【`、`】`。
...
配列の長さは文字列の字数と同様です。詳しくは「文字列の長さ」より参照してください。
...
関数名は動詞（又は動詞句）で、同じスコープに重複の定義（ビルトイン関数も含まれ）は出来ません（※）。関数の中身は一個のホワイトスペース（全角・半角スペースまたはタブ）のインデントが必要です。詳しくは「インデント」より参照してください。反復内には関数の定義が不可能です。
