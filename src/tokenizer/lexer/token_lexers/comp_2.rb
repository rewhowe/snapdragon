  ...
  class Lexer
    module TokenLexers
      ...
      def tokenize_comp_2(chunk, options = { reverse?: false })
        comparison_tokens = {
          Token::QUESTION      => [Token.new(Token::COMP_EQ)],   # もし A？ ならば・もし 関数呼び出す？ ならば
          Token::BANG          => [Token.new(Token::COMP_EQ)],   # もし A？ ならば・もし 関数呼び出す！ ならば
          Token::FUNCTION_CALL => [Token.new(Token::COMP_EQ)],   # もし 関数呼び出す ならば
